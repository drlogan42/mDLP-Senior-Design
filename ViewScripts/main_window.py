...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # =-= Main Layout (now HBox to include plots on right) =-=-
        main_layout = QHBoxLayout(central_widget)
        main_layout.setContentsMargins(10, 10, 10, 10)
        main_layout.setSpacing(10)

        # =-= Left Panel (VBox for all controls) =-=-
        left_panel = QWidget()
        ...
        # Row 2: 2 Buttons (spanning columns)
        ...
        plot_panel.setFixedSize(1000, 1000)  # width x height

        # Create 6 plots in 3x2 grid
        for row in range(3):
            for col in range(2):
